		...
		return (
			<div className="results">
				{this.props.items.map( (item,index) => {
					...
					return  (
						<Motion key={item.id} defaultStyle={{x:0, opacity:0, scale:0}} style={{x: spring(rotateEnd,[1,1]), opacity:spring(1,[1,1]), scale:spring(1, [1,1])}}>
			    		...
			    		{({x, opacity, scale}) => 
			    			<div className="result-card col-xs-6 col-sm-4" key={item.id} item={item} 
				    			...
				    			<button className="btn-view-image" onClick={() => this.props.selectListItem(item)}>View</button>
	           					<img src={item.webformatURL} alt={item.id} />
    						</div>
    					}
